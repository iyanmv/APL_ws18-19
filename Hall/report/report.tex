...
\begin{equation*}
D_V(E)=\frac{\sqrt{2}}{2\pi^2\hbar^3}\left(m_{lh}^{3/2}+m_{hh}^{3/2}\right)\sqrt{E_V-E}
\end{equation*}
where $m_{lh}$ and $m_{hh}$ are the mass of light and heavy holes (explained in the next subsection).

Finally, defining 
\begin{equation}
N_V=\frac{2}{h^3}\left(m_{lh}^{3/2}+m_{hh}^{3/2}\right)(2\pi k_BT)^{3/2}
\end{equation}
we can write the following formula for the carriers density:
\begin{equation}
p=\frac{2\,N_V}{\sqrt{\pi}}F_{1/2}\left(\frac{E_V-E_F}{k_BT}\right)
\end{equation} where $F_{1/2}(y)=\int_0^\infty\frac{x^{1/2}}{1+e^{(x-y)}}dx$ is the Fermi-Dirac integral and $E_F$ is the Fermi energy. In the case $k_BT\ll E_F-E_V$ the solution of the integral is:
\begin{equation}\label{eq::p}
p=N_V\exp\left(-\frac{E_F-E_V}{k_BT}\right)
\end{equation}

In our case the semiconductor is p-doped. This means that a concentration $N_A^-$ of ionized acceptors has been added to the material. We can express this concentration of ionized acceptors as:
\begin{equation}\label{eq::Na-}
N_A^-=\frac{N_A}{1+4\exp\left(\frac{E_A-E_F}{k_BT}\right)}\,,
\end{equation}
where $E_A$ is the energy of bound acceptor holes.

The material needs to respect the neutrality of charge, i.e. $p=n+N_A^-$. The intrinsic conductivity is neglectable for a certain range of temperatures, in this case we can write the charge neutrality as $p=N_A^-$. Hence, using equations \eqref{eq::p} and \eqref{eq::Na-} we obtain:
\begin{equation}
p\simeq\frac{N_A}{1+4(p/N_V)\exp\left(\frac{E_A-E_V}{k_BT}\right)}
\end{equation}
Solving the equation for $p$, we get:
\begin{equation}
p=\simeq\frac{2N_A}{1+\sqrt{1+16\,\dfrac{N_A}{N_V}\exp\left(\dfrac{E_A-E_V}{k_BT}\right)}}
\end{equation}For low temperatures ($T\ll\frac{E_A-E_V}{k_B}$) we can simplify the previous expression as:
\begin{equation}
p\simeq \frac{1}{2}\sqrt{N_AN_V}\exp\left(-\frac{E_A-E_V}{2K_BT}\right)
\end{equation}
...
\end{equation}
We notice that the energy is double degenerate. The effective mass in the $+$ case is called \emph{heavy hole mass} ($m_{hh}$) and in the $-$ case is called \emph{light hole mass} ($m_{lh}$).
