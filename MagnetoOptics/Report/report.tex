...
These previous expressions are the ones we will utilize during the experiment, but if we want to understand microscopically the origin of these effects, we need to consider Maxwell's equations. In particular, we need to understand the relations between the two curl equations
\begin{align}\label{eq:maxwell1}
\curl{\vec{E}}&=-\pdv{\vec{B}}{t}=-\mu_0\pdv{\vec{H}}{t}\\\label{eq:maxwell2}
\curl{\vec{H}}&=\epsilon_0\pdv{\vec{E}}{t}+\vec{J}\,,
\end{align}
where the current density is given by
\begin{equation}\label{eq:current_density}
\vec{J}=\pmb{\chi}\pdv{E}{t}\,,
\end{equation}
and $\pmb{\chi}$ is the polarizability tensor. The first equality in \eqref{eq:maxwell1} is true when the relative permeability $\mu/\mu_0\approx1$ and we can write $\vec{B}=\mu_0\vec{H}$. After this considerations, \eqref{eq:maxwell2} can be written in terms of the polarizability tensor
\begin{equation}\label{eq:maxwell2_tensor}
\curl{\vec{H}}=\epsilon_0(\pmb{1}+\pmb{\chi})\pdv{\vec{E}}{t}
\end{equation}
And it is precisely the non-scalar character of this expression, due to $\pmb{\chi
}$, which makes magneto-optical effects appear under certain applied magnetic fields or the magnetic state of the material.
